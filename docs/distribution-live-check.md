...
- `windows_real_machine_smoke`：仍为 open。当前环境可以执行查询脚本，但匿名 GitHub API 查询被 rate limit 拦截，尚未确认 `windows-package.yml` 最新 run 或 `yuzai-windows-package` artifact。
...
1. 使用 `GITHUB_TOKEN=只读令牌 npm run actions:windows-status` 查询 Windows workflow run 和 artifact。
2. 下载或生成 Windows 安装包后，在 Windows 10/11 环境执行 `docs/windows-release-smoke.md`。
3. 配置正式 Developer ID Application 签名身份。
4. 使用正式签名配置重新构建 macOS 包，并完成 notarization。
5. 在签名/公证包上复核普通用户首次打开、安全提示、安装和卸载体验。
6. 补充 closureEvidence 后，重新运行 `npm run validate:all` 和 `npm run validate:release`。
