...
## 2026-06-27 C1 互动动作专用过渡视频接入 runtime

日期：2026-06-27
源文件：`docs/runtime-intake-waves.json`、`docs/runtime-intake-approvals/c1-interaction-transitions.approved.json`、`docs/runtime-intake-c1-interaction-transitions-dry-run.md`、`assets/runtime/animations/manifest.json`、`assets/runtime/animations/idle_primary_to_paw_raise/frames`、`assets/runtime/animations/paw_raise_to_idle_primary/frames`、`assets/runtime/animations/idle_primary_to_cursor_watch/frames`、`assets/runtime/animations/cursor_watch_to_idle_primary/frames`、`assets/runtime/animations/idle_primary_to_click_surprised/frames`、`assets/runtime/animations/click_surprised_to_idle_primary/frames`、`assets/runtime/animations/idle_primary_to_poke_annoyed/frames`、`assets/runtime/animations/poke_annoyed_to_idle_primary/frames`
问题：用户继续要求执行下一步，需要把已生成的 C1 过渡视频真正接入桌宠 runtime，用真实序列帧替代互动动作进入和退出时的硬切。
处理：新增 `c1-interaction-transitions` runtime 接入波次和批准文件；执行 `npm run runtime:intake-executor -- --wave c1-interaction-transitions --execute`，为 8 个过渡动作各抽取 120 帧，处理绿幕抠像、水印区域透明化和绿色残留清理。将 8 个 transition 动作写入 manifest，并把 `paw_raise`、`cursor_watch`、`click_surprised`、`poke_annoyed` 的 `transitionIn` / `transitionOut` 指向对应 C1 过渡动作。随后根据 `docs/action-transition-risk-report.md` 的 best source frame，把 C1 过渡动作的 `exitFrames` 收紧到相似帧锚点：`idle_primary_to_paw_raise=28`、`paw_raise_to_idle_primary=10`、`idle_primary_to_cursor_watch=3`、`cursor_watch_to_idle_primary=13`、`click_surprised_to_idle_primary=14`、`idle_primary_to_poke_annoyed=2`、`poke_annoyed_to_idle_primary=12`；`idle_primary_to_click_surprised` 因最佳点为第 1 帧，暂不缩短。
验证命令：`npm run validate:c1-transition-runtime-intake`、`npm run validate:runtime-animations`、`npm run validate:runtime-alpha-quality`、`npm run animations:transition-risk -- --write docs/action-transition-risk-report.md`、`npm run animations:asset-contract -- --write docs/animation-asset-contract.md`。
桌面验收：沙盒内启动 Electron 仍会因 GUI 权限返回 `SIGABRT`，用提升权限重跑后成功。直接预览 `idle_primary_to_paw_raise` 时，早期 420ms 截图因帧尚未预加载完成只得到 1 个变化帧；延迟到 1400ms 后，`npm run capture:inspect -- --sequence-path /private/tmp/yuzai-window-c1-paw-late.png --count 10 --min-changed-frames 6 --min-width 200 --min-height 200` 通过，`changedFrames=10`。预览 `paw_raise` 链路并应用 C1 锚点后，`npm run capture:inspect -- --sequence-path /private/tmp/yuzai-window-c1-paw-chain-anchored.png --count 18 --min-changed-frames 8 --min-width 200 --min-height 200` 通过，`changedFrames=13`。桌面证据图保存到 `assets/reviews/runtime/c1-interaction-transitions/`。
风险复查：C1 接入后再按相似帧锚点优化，动作衔接风险报告从 `high 7 / medium 47 / low 22` 降为 `high 4 / medium 43 / low 29`；C1 自身回待机动作已降为 low，剩余 high 集中在 `sleep`、`waking`、`poke_annoyed`、`paw_raise` 的理论回切项，报告中均已有对应 bridge，需要继续结合桌面录屏判断真实观感。
已知问题：当前 C1 源视频姿态幅度偏保守，尤其抬爪链路更像头部和身体轻微调整，而不是明显大幅抬爪；若用户继续觉得“不像互动”，应重新生成幅度更明显的 C1 过渡或延长互动本体展示窗口。
决定：接受 C1 runtime 接入和第一轮相似帧锚点优化作为第二层修复；它已经把过渡动作接入真实桌面播放链路，并降低 C1 自身回切风险，但仍不等同于最终动作美术优化完成。
