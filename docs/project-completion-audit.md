...
用途：把当前项目距离“本机自用第一版桌宠”的证据、剩余风险和下一步边界放到一个中文入口。本文档不替代 `docs/project-status.md`，也不批准生成视频、抽帧、覆盖 runtime 或修改 manifest。
...
- Windows 实机验收清单：`docs/windows-release-smoke.md` 已保留为未来对外分发参考；Windows 实机验收已取消为本机自用完成阻塞。
- Windows 远端打包入口：`.github/workflows/windows-package.yml` 已保留，后续如恢复 Windows 分发可继续使用。
- Windows Actions 状态查询：`docs/windows-actions-status.md` 已保留 `npm run actions:windows-status` 只读查询入口。
- Windows Actions 手动触发：`docs/windows-actions-dispatch.md` 已保留 `npm run actions:windows-dispatch` 入口。
- macOS 签名与公证已取消：当前只在本人电脑本机运行，不要求 Developer ID、notarization 或 Gatekeeper 普通用户分发验收。
- 签名后普通用户安全提示复核已取消：该项随 macOS 对外分发取消，不再作为本机自用完成阻塞。
- 剩余硬缺口清单：`docs/release-blockers.json` 已记录 `transition_out_high_risk`、`runtime_duration_short` 为 closed，并记录 `windows_real_machine_smoke`、`macos_sign_notarize`、`signed_user_safety_recheck` 为 canceled；中文报告见 `docs/release-blockers.md`。
- 项目可标记完成：是。本结论仅适用于用户确认的“只在本人电脑上跑”的本机自用口径。
...
| Windows 安装包实机验收清单 | `docs/windows-release-smoke.md` | 已保留，已取消为本机自用阻塞 |
| Windows 远端试用包工作流 | `.github/workflows/windows-package.yml` | 已保留，未来分发可复用 |
| Windows Actions 状态查询入口 | `docs/windows-actions-status.md` | 已保留 |
| Windows Actions 手动触发入口 | `docs/windows-actions-dispatch.md` | 已保留 |
| 发布阻塞现场检查记录 | `docs/distribution-live-check.md` | 已更新为本机自用口径 |
| 剩余硬缺口机器清单 | `docs/release-blockers.json`、`docs/release-blockers.md` | 已同步，本机自用无 open blocker |
...
当前已通过本地 `package:dir` 打包验证，并已补齐面向本机自用和未来试用分发的安装/回滚说明、卸载说明和版本标签规范：`docs/release-playbook.md`。实际试用标签记录在 `docs/release-tag-record.md`，签名包安全提示见 `docs/signed-release-safety.md`，macOS 签名/公证状态见 `docs/macos-signing-notarization-status.md`，Windows 实机验收清单见 `docs/windows-release-smoke.md`，Windows 远端试用包工作流见 `.github/workflows/windows-package.yml`，Windows Actions 状态查询见 `docs/windows-actions-status.md`，本轮发布阻塞现场检查见 `docs/distribution-live-check.md`。

本轮范围变更结论：用户已确认 Windows 实机验收、macOS 签名公证取消，只需要在本人电脑上跑。因此 `windows_real_machine_smoke`、`macos_sign_notarize`、`signed_user_safety_recheck` 均为 canceled，不再阻塞本机自用完成。

- Windows 实机验收已取消。
- macOS 签名与公证已取消。
- 签名后普通用户安全提示复核已取消。

机器可读 canceled blocker：`windows_real_machine_smoke`、`macos_sign_notarize`、`signed_user_safety_recheck`，记录于 `docs/release-blockers.json`。

## 下一步建议

1. 如果未来目标改为发布给别人试用，应重新启用签名、公证和 Windows 实机验收，并按 `docs/release-playbook.md` 创建可回滚版本标签。
