...
- 多帧动画验收：已接入 `YUZAI_CAPTURE_SEQUENCE_PATH`、`YUZAI_CAPTURE_SEQUENCE_COUNT` 和 `YUZAI_CAPTURE_SEQUENCE_INTERVAL_MS` 测试钩子，可一次捕获连续桌面帧，用于检查播放过程中的空白帧、闪烁、卡顿和姿势切换；路径规划已通过 `npm run validate:capture-plan` 验证，截图序列存在性、非空和变化检查已通过 `npm run validate:capture-sequence-inspector` 验证。
