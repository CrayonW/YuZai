...
用途：说明当前未签名本机自用包和未来正式签名与公证包的安全边界，避免把本机验证包误当成正式分发包。
...
- 当前包形态：未签名试用包。
- 当前分发方式：仅在本人电脑本机运行。
- 当前不代表正式签名与公证发布；按 2026-06-27 用户确认，macOS 签名公证已取消为项目完成阻塞。
- 当前签名/公证状态报告：`docs/macos-signing-notarization-status.md`。
...
- 已知限制：当前 4 个 high 风险回切已接入 transitionOut 并完成桌面截图验收；runtime 时长不足动作数为 0，但 phase1-d 方向跟随动作仍是基于已清理 runtime 帧循环展开，不等于真正更长源视频。
