...
| medium | 进入 | idle_primary | idle_primary_to_cursor_watch | 96 -> 1 | 0.1247 | 4 (0.0909) | not-applicable | Canvas crossfade | 保留 Canvas crossfade，并用多帧截图复查 | idle_primary_to_cursor_watch 是姿势链路过渡动作 |
...
| medium | 进入 | idle_primary | paw_raise_to_idle_primary | 96 -> 1 | 0.1243 | 4 (0.0896) | not-applicable | Canvas crossfade | 保留 Canvas crossfade，并用多帧截图复查 | paw_raise_to_idle_primary 是姿势链路过渡动作 |
| medium | 进入 | idle_primary | idle_primary_to_poke_annoyed | 96 -> 1 | 0.1243 | 4 (0.0908) | not-applicable | Canvas crossfade | 保留 Canvas crossfade，并用多帧截图复查 | idle_primary_to_poke_annoyed 是姿势链路过渡动作 |
| medium | 进入 | idle_primary | idle_primary_to_paw_raise | 96 -> 1 | 0.1242 | 4 (0.0909) | not-applicable | Canvas crossfade | 保留 Canvas crossfade，并用多帧截图复查 | idle_primary_to_paw_raise 是姿势链路过渡动作 |
| medium | 进入 | idle_primary | click_surprised | 96 -> 1 | 0.1237 | 7 (0.0900) | not-applicable | idle_primary_to_click_surprised | 已配置 idle_primary_to_click_surprised，需桌面录屏确认 | click_surprised 是可被用户触发的交互动作 |
| medium | 进入 | idle_primary | cursor_watch_to_idle_primary | 96 -> 1 | 0.1235 | 4 (0.0903) | not-applicable | Canvas crossfade | 保留 Canvas crossfade，并用多帧截图复查 | cursor_watch_to_idle_primary 是姿势链路过渡动作 |
...
| medium | 回切 | click_surprised | idle_primary | 120 -> 96 | 0.1217 | 9 (0.0621) | acceptable | click_surprised_to_idle_primary | 已配置 click_surprised_to_idle_primary，需桌面录屏确认 | click_surprised 播放结束后回到 idle_primary |
...
| medium | 进入 | idle_primary | poke_annoyed_to_idle_primary | 96 -> 1 | 0.1159 | 4 (0.0807) | not-applicable | Canvas crossfade | 保留 Canvas crossfade，并用多帧截图复查 | poke_annoyed_to_idle_primary 是姿势链路过渡动作 |
...
| medium | 进入 | idle_primary | click_surprised_to_idle_primary | 96 -> 1 | 0.1141 | 4 (0.0788) | not-applicable | Canvas crossfade | 保留 Canvas crossfade，并用多帧截图复查 | click_surprised_to_idle_primary 是姿势链路过渡动作 |
...
| medium | 进入 | idle_primary | poke_annoyed | 96 -> 1 | 0.1134 | 4 (0.0754) | not-applicable | idle_primary_to_poke_annoyed | 已配置 idle_primary_to_poke_annoyed，需桌面录屏确认 | poke_annoyed 是可被用户触发的交互动作 |
...
| medium | 回切 | idle_primary_to_click_surprised | click_surprised | 120 -> 1 | 0.1095 | 1 (0.0565) | acceptable | Canvas crossfade | 保留 Canvas crossfade，并用多帧截图复查 | idle_primary_to_click_surprised 播放结束后回到 click_surprised |
...
| medium | 回切 | cursor_watch | idle_primary | 120 -> 1 | 0.1042 | 22 (0.0775) | acceptable | cursor_watch_to_idle_primary | 已配置 cursor_watch_to_idle_primary，需桌面录屏确认 | cursor_watch 播放结束后回到 idle_primary |
...
| low | 进入 | idle_primary | paw_raise | 96 -> 1 | 0.0991 | 2 (0.0513) | not-applicable | idle_primary_to_paw_raise | 已配置 idle_primary_to_paw_raise，需桌面录屏确认 | paw_raise 是可被用户触发的交互动作 |
...
| low | 回切 | idle_primary_to_paw_raise | paw_raise | 28 -> 1 | 0.0888 | 28 (0.0888) | acceptable | Canvas crossfade | 当前可接受，低频复查即可 | idle_primary_to_paw_raise 播放结束后回到 paw_raise |
...
| low | 回切 | cursor_watch_to_idle_primary | idle_primary | 13 -> 1 | 0.0717 | 13 (0.0717) | acceptable | Canvas crossfade | 当前可接受，低频复查即可 | cursor_watch_to_idle_primary 播放结束后回到 idle_primary |
...
| low | 回切 | click_surprised_to_idle_primary | idle_primary | 14 -> 1 | 0.0669 | 14 (0.0669) | acceptable | Canvas crossfade | 当前可接受，低频复查即可 | click_surprised_to_idle_primary 播放结束后回到 idle_primary |
| low | 回切 | poke_annoyed_to_idle_primary | idle_primary | 12 -> 1 | 0.0652 | 12 (0.0652) | acceptable | Canvas crossfade | 当前可接受，低频复查即可 | poke_annoyed_to_idle_primary 播放结束后回到 idle_primary |
| low | 回切 | paw_raise_to_idle_primary | idle_primary | 10 -> 1 | 0.0638 | 10 (0.0638) | acceptable | Canvas crossfade | 当前可接受，低频复查即可 | paw_raise_to_idle_primary 播放结束后回到 idle_primary |
| low | 回切 | idle_primary_to_poke_annoyed | poke_annoyed | 2 -> 1 | 0.0457 | 2 (0.0457) | acceptable | Canvas crossfade | 当前可接受，低频复查即可 | idle_primary_to_poke_annoyed 播放结束后回到 poke_annoyed |
| low | 回切 | idle_primary_to_cursor_watch | cursor_watch | 3 -> 1 | 0.0439 | 3 (0.0439) | acceptable | Canvas crossfade | 当前可接受，低频复查即可 | idle_primary_to_cursor_watch 播放结束后回到 cursor_watch |
